...
export async function createFakeSignature() {
  const name = faker.name.findName();
  const nationalId = createSsn();
  const comment = Math.random() < 0.5 ? faker.lorem.sentence() : '';
  const anonymous = Math.random() < 0.5;

  const weeks = 604800000 * 2;
  const d1 = new Date(Date.now() - weeks);
  const d2 = new Date(Date.now());
  const signed = faker.date.between(d1, d2);
